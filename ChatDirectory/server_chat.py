...
def sending_frames_thread(room, client_s):
    enc_frame = my_funcs.recvall(client_s)
    while enc_frame:
...
def session_with_client(client_socket):  # start the session
    ...
    enc_ans = my_funcs.recvall(client_socket)
    if enc_ans == -999:
        return
    ...
    while not success and ans == '2':   # create account
        first_name = decrypt_cipher(my_funcs.recvall(client_socket), client_seeds[client_socket][3])
        # print(first_name)
        if first_name == '0':
            break
        last_name = decrypt_cipher(my_funcs.recvall(client_socket), client_seeds[client_socket][3])
        # print(last_name)
        if last_name == '0':
            break
        user_name = decrypt_cipher(my_funcs.recvall(client_socket), client_seeds[client_socket][3])
        # print(user_name)
        if user_name == '0':
            break
        password = decrypt_cipher(my_funcs.recvall(client_socket), client_seeds[client_socket][3])
        # print(password)
        if password == '0':
            break
    ...
    while not success:   # login
        ...
        password = decrypt_cipher(my_funcs.recvall(client_socket), client_seeds[client_socket][3])
        # print(password)
        if user_name in clients_users and \
                (Fernet(showing_key_of_specific_rec_client_password_key_table(user_name).encode())
                        .decrypt(showing_password_of_specific_rec_client_user_table(user_name).encode())).decode() == password and user_name not in clients:
            success = True
    ...
    enc_room = my_funcs.recvall(client_socket)
    ...
    while flag_session:
        ...
        if [user_name, 3] in private_sessions.values():    # part 5                                                # creator
            for tuple_private in private_sessions.items():    # tuple_private = (sec_user, me)
                if tuple_private[1][0] == user_name:
                    ...
                    break
            print('c')
            enc_private_msg_from_client = my_funcs.recvall(client_socket)
        ...
        if user_name in private_sessions.keys() and private_sessions[user_name][1] == 3:   # part 5                # second
            ...
            if [user_name, 1] in private_sessions.values():   # part 3
                for (sec_user, me) in private_sessions.items():
                    if me[0] == user_name:
                        break
                print('***')
                public_key_private = my_funcs.recvall(client_socket)
                print(f'public key from c {public_key_private};')
                pq_private = my_funcs.recvall(client_socket)
            ...
            if user_name in private_sessions.keys() and private_sessions[user_name][1] == 2:   # part 4
                print('****')
                enc_seed = my_funcs.recvall(client_socket)
            ...
            if [user_name, 0] in private_sessions.values() or [user_name, 1] in private_sessions.values() or [user_name, 2] in private_sessions.values() or [user_name, 3] in private_sessions.values():
                ...
                enc_msg_from_client = my_funcs.recvall(client_socket)
                ...
                if user_name in private_sessions.keys():   # part 2
                    ...
                    if msg_from_client == '!online':  # show the connected clients
                        ...
                        enc_file_name = my_funcs.recvall(client_socket)
                        ...
                        enc_file_name_hmac = my_funcs.recvall(client_socket)
                        ...
                        if hmac.new(str(client_seeds[client_socket][2]).encode(), file_name.encode(), hashlib.sha256).hexdigest() == file_name_hmac:    # check the integrity of the file name
                            ...
                            enc_block_file = my_funcs.recvall(client_socket)
                            ...
                            enc_body_file_hmac = my_funcs.recvall(client_socket)
                        ...
                        enc_to_who = my_funcs.recvall(client_socket)
                        ...
                        enc_frame = my_funcs.recvall(client_socket)
                        # print('*')
                        while enc_frame:
                            # print('**')
                            for others_clients in current_room.values():
                        ...
                        if to_who in clients.keys() and to_who != user_name:
                            # enc_private_msg = my_funcs.recvall(client_socket)
